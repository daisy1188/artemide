...
export async function bootstrap(root){
  ...
  const entityCache = new Map(); // page -> entity[]
  const entityById = new Map();
  const inspector = createEntityInspector({
    toast,
    onRowHover:(r)=>setSelectedEntityIds([r.entityId], false),
    onRowLeave:()=>setSelectedEntityIds(store.getState().entityInspector.selectedIds || [], false)
  });
  ...
  function setSelectedEntityIds(ids, persist=true){
    if (persist) setEntityState({ selectedIds: [...new Set(ids)] }, false);
    redraw();
  }

  function toggleEntitySet(id, field){
    const curr = new Set(getEntityState()[field] || []);
    if (curr.has(id)) curr.delete(id); else curr.add(id);
    setEntityState({ [field]: [...curr] }, false);
    redraw();
  }

  function zoomToEntity(rec){
    if (!rec || rec.page == null) return;
    if (viewer.pageNo !== rec.page) {
      viewer.pageNo = rec.page;
      viewer.render().then(redraw);
    }
    const c = { x: (rec.centerX ?? rec.x ?? 0) * currentScale(), y: (rec.centerY ?? rec.y ?? 0) * currentScale() };
    host.scrollLeft = Math.max(0, c.x - host.clientWidth / 2);
    host.scrollTop = Math.max(0, c.y - host.clientHeight / 2);
  }
  ...
  async function ensureEntities(scope='current', force=false){
    ...
    for (const p of pages){
      ...
      entityCache.set(p, extracted);
      all.push(...extracted);
      toast(`Entity extraction p.${p}: ${extracted.length}`,'info');
    }
  ...
  const redraw=()=>{
    fitStack();
    renderOverlays(overlayCtx,store.getState(),viewer.pageNo,overlayCtrl.selected,{
      scale:currentScale(),
      entityDebug:{
        entitiesById:Object.fromEntries(entityById.entries()),
        selectedIds:getEntityState().selectedIds||[],
        hiddenIds:getEntityState().hiddenIds||[],
        pinnedIds:getEntityState().pinnedIds||[],
        showPins:getEntityState().showPins!==false,
        showHiddenMasks:getEntityState().showHiddenMasks!==false
      }
    });
  ...
  async function openEntityInspector(){
    if(!viewer.doc) return toast('Carica prima un PDF','warn');
    inspector.open();
    await ensureEntities(getEntityState().scope || 'current', false);
  }
  ...
  function renderProps(){
    ...
    if(sel.geometry.rect){for(const k of ['x','y','w','h']){propsEl.append(field(k,'number',sel.geometry.rect[k],(v)=>actions.updateOverlay(store,sel.id,{geometry:{...sel.geometry,rect:{...sel.geometry.rect,[k]:Number(v)}}})));}}
    ...
    if(sel.type==='box'){
      ...
      const styleSel=document.createElement('select'); styleSel.className='input'; ['solid','dashed','dotted'].forEach(v=>{const o=document.createElement('option');o.value=v;o.textContent=v;o.selected=(sel.style.stroke.style===v);styleSel.append(o);}); styleSel.onchange=(e)=>actions.updateOverlay(store,sel.id,{style:{...sel.style,stroke:{...sel.style.stroke,style:e.target.value}}});
    ...
    if(sel.type==='note') propsEl.append(field('Text','text',sel.content.text,(v)=>actions.updateOverlay(store,sel.id,{content:{...sel.content,text:v}})));
  }
  ...
  inspector.mount({
    ...
    selectRow: (id,e)=>{
      ...
      setSelectedEntityIds([...set], true);
      const rec = entityById.get(id); if (rec) zoomToEntity(rec);
    },
    selectAll:(ids)=>setSelectedEntityIds(ids,true),
    clearSelection:()=>setSelectedEntityIds([],true),
    zoomTo: zoomToEntity,
